<%@ Control Language="c#" Inherits="ASPNET.StarterKit.Portal.DesktopPortalBanner" CodeFile="DesktopPortalBanner.ascx.cs" %>
...
<table width="100%" cellspacing="0" class="HeadBg" border="0">
	<tr valign="top">
		<td colspan="3" class="SiteLink" background="<%= Global.GetApplicationPath(Request) %>/images/bars.gif" align="right">
			...
			<a href="<%= Global.GetApplicationPath(Request) %>/Default.aspx" class="SiteLink">Portal Home</a>
			<%= LogoffLink %>
			&nbsp;&nbsp;
		</td>
	</tr>
	...
	<tr>
		<td>
			<asp:datalist id="tabs" cssclass="OtherTabsBg" repeatdirection="horizontal" ItemStyle-Height="25" SelectedItemStyle-CssClass="TabBg" ItemStyle-BorderWidth="1" EnableViewState="false" runat="server">
				<ItemTemplate>
					&nbsp;<a href='<%= Global.GetApplicationPath(Request) %>/Default.aspx?tabindex=<%# Container.ItemIndex %>&tabid=<%# ((TabStripDetails) Container.DataItem).TabId %>' class="OtherTabs"><%# ((TabStripDetails) Container.DataItem).TabName %></a>&nbsp;
				</ItemTemplate>
